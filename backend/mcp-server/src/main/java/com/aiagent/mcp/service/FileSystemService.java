...
public class FileSystemService {
    ...
    public void writeFile(String path, String content) throws IOException {
        Path normalizedPath = normalizeAndValidateWritePath(path);

        if (content.length() > maxFileSize) {
            throw new IOException("Content size " + content.length() + " exceeds maximum allowed size of " + maxFileSize);
        }

        Files.writeString(normalizedPath, content, StandardCharsets.UTF_8,
    ...
    private Path normalizeAndValidateWritePath(String path) throws IOException {
        if (!StringUtils.hasText(path)) {
            throw new IOException("Path cannot be null or empty");
        }
